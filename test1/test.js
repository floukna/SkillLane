...
function find_largest(floor, n) {
  let list = [floor * floor]
  let round = Math.ceil(floor / 2)
  console.log(round);
  let exponent = 3
  while (round > 1) {
...
function largest_power(n) {
  if (between(n, 2, 4)) {
    return [1, -1]
  } else if (between(n, 1, 1000000) && n !== 1) {
    const sqrt = Math.sqrt(n)
    let floor = Math.floor(sqrt)
    if (isFloat(sqrt)) {
      return find_largest(floor, n)
    } else {
      return find_largest(floor - 1, n)
    }
  }
  return [0, -1]
}
